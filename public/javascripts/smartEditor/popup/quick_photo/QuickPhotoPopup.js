  //변수 선언 및 초기화
    ...
    function html5Upload() {	
    	var elImage,
    		...
    		tempFile,
    		sUploadURL;
    	sUploadURL= '/quick'; 	//upload URL
    	quickFileUploadParams.max  = 0;
      for (var i in htImageInfo) {
        if (htImageInfo.hasOwnProperty(i)) quickFileUploadParams.max ++;
      }
      console.log(quickFileUploadParams.max );
      quickFileUploadParams.count = 0;
    	//파일을 하나씩 보내고, 결과를 받음.
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
                ...
                if (quickFileUploadParams.count == quickFileUploadParams.max) {
                  window.close();  
                }
	    				},
	    				timeout : 3,
	    			...
	    			oAjax.header("Content-Type","application/octet-stream");
	    			oAjax.header("file-name",encodeURIComponent(tempFile.name.replace(/\s/g,'_')));
    ...
    function makeArrayFromString(sResString){
      ...
      quickFileUploadParams.count++;
      resJson = JSON.parse(sResString);
            
      quickFileUploadParams.fileArray.push(resJson);
 		  if (quickFileUploadParams.count == quickFileUploadParams.max) {
        setPhotoToEditor(quickFileUploadParams.fileArray);  
 		  }
    }
